unit UMemoria_Fita_Detalhe_Arquivo;
...
type
  TFrmMemoria_Fita_Detalhe_Arquivo = class(TForm)
...
var
  FrmMemoria_Fita_Detalhe_Arquivo: TFrmMemoria_Fita_Detalhe_Arquivo;
...
procedure TFrmMemoria_Fita_Detalhe_Arquivo.BBtnFecharClick(
  Sender: TObject);
begin
  close;
end;

procedure TFrmMemoria_Fita_Detalhe_Arquivo.BBtnSalvarClick(
  Sender: TObject);
begin
...
procedure TFrmMemoria_Fita_Detalhe_Arquivo.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
  FrmMemoria_Fita_Detalhe_Arquivo.Free;
  FrmMemoria_Fita_Detalhe_Arquivo:= Nil;
end;

procedure TFrmMemoria_Fita_Detalhe_Arquivo.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmMemoria_Fita_Detalhe_Arquivo.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key=#13 then
    begin
...
procedure TFrmMemoria_Fita_Detalhe_Arquivo.GeraArquivoMDF;
var
  aux: string;
...
//        cDataInicial:= MEdtData_Inicial.Text;
//        cDataFinal:= MEdtData_Final.Text;
//        dm.ACBrECF1.PafMF_MFD_Cotepe1704(StrToDateTime(cDataInicial), StrToDateTime(cDataFinal), cArquivoOrigem);
//      end
//    else
//      begin
//        cTipo:= 'C';
//        cDataInicial:= EdtInicio.Text;
//        cDataFinal:= EdtFim.Text;
//        dm.ACBrECF1.PafMF_MFD_Cotepe1704(StrToInt(cDataInicial), StrToInt(cDataFinal), cArquivoOrigem);
//      end;}
//      dm.ACBrECF1.ArquivoMFD_DLL(cArquivoOrigem);
//
//    //cUsuario:= '1';
//    //iTipoGeracao:= 1;
//    //iUnicoArquivo:= 1;
...
//    UDeclaracao.Insere_Historico(usuario, 'GEROU ARQUIVO ELETRÔNICO DA MFD.', TimeToStr(time), consu, date);
//  except
//    on E: Exception do
...
procedure TFrmMemoria_Fita_Detalhe_Arquivo.GeraEspelhoMDF;
begin
  try
...
procedure TFrmMemoria_Fita_Detalhe_Arquivo.RGOpcaoClick(Sender: TObject);
begin
  if (RGOpcao.ItemIndex = 0) then
  begin
...
function TFrmMemoria_Fita_Detalhe_Arquivo.ValidaIntervalo: Boolean;
begin
  Result:= False;
