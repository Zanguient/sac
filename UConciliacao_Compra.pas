...
type
  TFrmConciliacao_Compra = class(TForm)
    ...
    qryitens_compraCodigo_Venda: TStringField;
  ...
  private
    Conciliacao_Compra: URegistro.TConciliacao_Compra;
    Util: TUtil;
    qtde_itens: integer;
  public
    ativo: boolean;
...
uses UDM, UChama_Produto, UChama_Cliente, UChama_Condicao_Pagamento,
  UDeclaracao, UChama_Fornecedor, UChama_Funcionario, UChama_Produto_Compra;
...
begin
  ...
  begin
    close;
    clear;
    add('select CC.*, P.Codigo_Venda, P.Descricao, UM.Sigla from Conciliacao_Compra CC');
    add('inner join Produto P on (CC.Codigo_Produto = P.Codigo)');
...
begin
  ...
  BBtnSalvar.Enabled:= false;
  BBtnCancelar.Enabled:= false;
  UDeclaracao.Insere_Historico(usuario, 'REALIZOU CONCILIAÇÃO DE COMPRAS ' + EdtN_Pedido.Text + '.', TimeToStr(time), alteracao, date);
  Util.Desabilita_Campos(FrmConciliacao_Compra);
end;
...
begin
  ...
  begin
    if (Confirma_Delete = true) then
      begin
        Conciliacao_Compra.Excluir(Conciliacao_Compra);
      end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Conciliacao_Compra.Inserir(Conciliacao_Compra);
      Atualiza_Dados;
      Limpa_Menor;
...
begin
  ...
  if (qAux2.FieldByName('CPF').AsString <> '') then
    FrmConciliacao_Compra.LblCPF.Caption:= qAux2.FieldByName('CPF').AsString
  else if (qAux2.FieldByName('CNPJ').AsString <> '') then
    FrmConciliacao_Compra.LblCPF.Caption:= qAux2.FieldByName('CNPJ').AsString
  else
    FrmConciliacao_Compra.LblCPF.Caption:= '';

  FrmConciliacao_Compra.LblCidade.Caption:= qAux2.FieldByName('Descricao').AsString;
  FrmConciliacao_Compra.LblSetor.Caption:= qAux2.FieldByName('Descricao_1').AsString;
  FrmConciliacao_Compra.LblEndereco.Caption:= qAux2.FieldByName('Descricao_2').AsString;
  FrmConciliacao_Compra.LblTipo_Logradouro.Caption:= qAux2.FieldByName('Tipo_Logradouro').AsString;
  FrmConciliacao_Compra.LblUF.Caption:= qAux2.FieldByName('UF').AsString;
  FrmConciliacao_Compra.LblTelefone.Caption:= qAux2.FieldByName('Telefone').AsString;
  FrmConciliacao_Compra.LblNumero.Caption:= qAux2.FieldByName('Numero').AsString;
