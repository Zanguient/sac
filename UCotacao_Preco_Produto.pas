...
type
  TFrmCotacao_Preco_Produto = class(TForm)
    ...
    qryitens_cotacaoCodigo_Venda: TStringField;
  ...
  private
    Cotacao: URegistro.TCotacao;
    Itens_Cotacao: URegistro.TItens_Cotacao;
    Util: TUtil;
...
uses UDM, UChama_Produto, UDeclaracao, UChama_Funcionario,
  URel_Cotacao_Preco_Produto;
...
begin
  ...
  begin
    close;
    clear;
    add('select IC.*, P.Codigo_Venda, P.Descricao, UM.Sigla from Itens_Cotacao_Preco_Produto IC');
...
begin
  if (qryitens_cotacao.IsEmpty = false) then
    Itens_Cotacao.Excluir(Itens_Cotacao);
...
begin
  ...
  begin
    Cotacao.Excluir(Cotacao);
    Itens_Cotacao.Excluir(Itens_Cotacao);
    UDeclaracao.Insere_Historico(usuario, 'EXCLUIU COTAÇÃO DE PREÇO DE PRODUTOS ' + EdtN_Cotacao.Text + '.', TimeToStr(time), exclusao, date);
    BBtnSalvar.Enabled:= false;
    BBtnExcluir.Enabled:= false;
    BBtnImprimir.Enabled:= false;
    BBtnCancelar.Enabled:= false;
...
begin
  if (achei = false) then
    begin
      ...
      BEGIN
        Cotacao.Inserir(Cotacao);
        UDeclaracao.Insere_Historico(usuario, 'CADASTROU COTAÇÃO DE PREÇO DE PRODUTOS ' + EdtN_Cotacao.Text + '.', TimeToStr(time), insercao, date);
      END
      else
        exit;
    end
    ...
    begin
      if (Confira_Pedido = true) then
      BEGIN
        Cotacao.Alterar(Cotacao);
        UDeclaracao.Insere_Historico(usuario, 'ALTEROU COTAÇÃO DE PREÇO DE PRODUTOS ' + EdtN_Cotacao.Text + '.', TimeToStr(time), alteracao, date);
      END
      else
        exit;
    end;
...
begin
  ...
  begin
    close;
    clear;
    add('Select CP.*, F.Nome from Cotacao_Preco_Produto CP');
...
begin
  ...
  begin
    if (Confirma_Delete = true) then
      begin
        Itens_Cotacao.Excluir_Selecionado(Itens_Cotacao);
...
procedure TFrmCotacao_Preco_Produto.EdtCodigo_FuncionarioKeyDown(
...
procedure TFrmCotacao_Preco_Produto.EdtFuncionarioKeyPress(Sender: TObject;
  var Key: Char);
begin
  Util.Somente_Apaga(key);
end;

procedure TFrmCotacao_Preco_Produto.EdtN_CotacaoKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  begin
    ...
    begin
      Itens_Cotacao.Inserir(Itens_Cotacao);
      Atualiza_Dados;
      Calcula_Total;
      Limpa_Menor;
