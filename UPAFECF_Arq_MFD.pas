...
type
  TFrmPAFECF_Arq_MFD = class(TForm)
...
var
  FrmPAFECF_Arq_MFD: TFrmPAFECF_Arq_MFD;
...
procedure TFrmPAFECF_Arq_MFD.BBtnFecharClick(
  Sender: TObject);
begin
  close;
end;

procedure TFrmPAFECF_Arq_MFD.BBtnSalvarClick(
  Sender: TObject);
begin
...
procedure TFrmPAFECF_Arq_MFD.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
  FrmPAFECF_Arq_MFD.Free;
  FrmPAFECF_Arq_MFD:= Nil;
end;

procedure TFrmPAFECF_Arq_MFD.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmPAFECF_Arq_MFD.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if key=#13 then
    begin
...
procedure TFrmPAFECF_Arq_MFD.GeraArquivoMDF;
var
  aux: string;
...
procedure TFrmPAFECF_Arq_MFD.GeraEspelhoMDF;
begin
  try
...
procedure TFrmPAFECF_Arq_MFD.RGOpcaoClick(Sender: TObject);
begin
  if (RGOpcao.ItemIndex = 0) then
  begin
...
function TFrmPAFECF_Arq_MFD.ValidaIntervalo: Boolean;
begin
  Result:= False;
