...
uses UGeral, UConsulta_Compra, UDeclaracao;
...
begin
  ...
  begin
    close;
    clear;
    add('select C.*, IC.*, Forn.Nome_Fantasia, Fun.Nome, CP.Descricao, P.Codigo_Venda, P.Descricao, UM.Sigla from Compra C');
    add('left join Itens_Compra IC on (C.N_Pedido = IC.N_Pedido)');
    add('left join Produto P on (IC.Codigo_Produto = P.Codigo)');
    add('left join Unidade_Medida UM on (P.Codigo_Unidade_Medida = UM.Codigo)');
    add('left join Fornecedor Forn on (C.Codigo_Fornecedor = Forn.Codigo)');
    add('left join Funcionario Fun on (C.Codigo_Funcionario = Fun.Codigo)');
