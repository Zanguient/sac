...
uses UGeral, UDeclaracao, UConsulta_Retorno_Cotacao_Preco;
...
begin
  ...
  if (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 1) or (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 2) or
     (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 5) or (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 6) or
     (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 7) or (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 8) then
  begin
  ...
  else if (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 3) or (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 4) or
          (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 5) or (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 6) or
          (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 7) or (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 8) then
  begin
    LblParametro.Enabled:= true;
  ...
  if (FrmConsulta_Retorno_Cotacao_Preco.RGConsulta_Por.ItemIndex = 0) then
    ...
    LblConsulta_Lancamento.Caption:= 'Funcionário / Data Retorno'
  ...
  begin
    close;
    clear;
    add('select RC.*, IRC.*, Forn.Nome_Fantasia, CP.Descricao as Cond_Pag, F.Nome, P.Codigo_Venda, P.Descricao, UM.Sigla from Retorno_Cotacao_Preco RC');
    add('inner join Itens_Retorno_Cotacao_Preco IRC on (RC.Codigo = IRC.Codigo)');
    add('inner join Fornecedor Forn on (RC.Codigo_Fornecedor = Forn.Codigo)');
    add('inner join Condicao_Pagamento CP on (RC.Codigo_Condicao_Pagamento = CP.Codigo)');
    add('inner join Funcionario F on (RC.Codigo_Funcionario = F.Codigo)');
    add('left join Produto P on (P.Codigo = IRC.Codigo_Produto)');
    ...
    begin
      sql.Add('where RC.Codigo_Empresa = :Empresa and RC.Codigo_Funcionario = :Codigo');
      Parameters.ParamByName('Empresa').Value:= UDeclaracao.codigo_empresa;
    ...
    begin
      sql.Add('where RC.Codigo_Empresa = :Empresa and RC.Codigo_Fornecedor = :Codigo');
      Parameters.ParamByName('Empresa').Value:= UDeclaracao.codigo_empresa;
      Parameters.ParamByName('Codigo').Value:= StrToInt(FrmConsulta_Retorno_Cotacao_Preco.EdtCodigo_Funcionario.Text);
      open;
    end
    ...
    begin
      sql.Add('where RC.Codigo_Empresa = :Empresa and RC.Codigo_Funcionario = :Codigo and RC.Data_Cotacao between :DI and :DF');
      Parameters.ParamByName('Empresa').Value:= UDeclaracao.codigo_empresa;
      Parameters.ParamByName('Codigo').Value:= StrToInt(FrmConsulta_Retorno_Cotacao_Preco.EdtCodigo_Funcionario.Text);
      Parameters.ParamByName('DI').Value:= StrToDateTime(FrmConsulta_Retorno_Cotacao_Preco.MEdtData_Inicial.Text);
      Parameters.ParamByName('DF').Value:= StrToDateTime(FrmConsulta_Retorno_Cotacao_Preco.MEdtData_Final.Text);
    ...
    begin
      sql.Add('where RC.Codigo_Empresa = :Empresa and RC.Codigo_Funcionario = :Codigo and RC.Data_Retorno between :DI and :DF');
      Parameters.ParamByName('Empresa').Value:= UDeclaracao.codigo_empresa;
      Parameters.ParamByName('Codigo').Value:= StrToInt(FrmConsulta_Retorno_Cotacao_Preco.EdtCodigo_Funcionario.Text);
      Parameters.ParamByName('DI').Value:= StrToDateTime(FrmConsulta_Retorno_Cotacao_Preco.MEdtData_Inicial.Text);
      Parameters.ParamByName('DF').Value:= StrToDateTime(FrmConsulta_Retorno_Cotacao_Preco.MEdtData_Final.Text);
