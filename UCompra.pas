...
uses UDM, UChama_Produto, UChama_Cliente, UChama_Condicao_Pagamento,
  UDeclaracao, UChama_Fornecedor, UChama_Funcionario, UEscolha_Cotacao;
...
begin
  ...
  begin
    close;
    clear;
    add('select IC.*, P.Codigo_Venda, P.Descricao, UM.Sigla from Itens_Compra IC');
...
begin
  if (qryitens_compra.IsEmpty = false) then
    Itens_Compra.Excluir(Itens_Compra);
...
begin
  ...
  begin
    ...
    Itens_Compra.Excluir(Itens_Compra);
    UDeclaracao.Insere_Historico(usuario, 'EXCLUIU PEDIDO DE COMPRA ' + EdtN_Pedido.Text + '.', TimeToStr(time), exclusao, date);
    BBtnSalvar.Enabled:= false;
    BBtnExcluir.Enabled:= false;
    BBtnCancelar.Enabled:= false;
    BBtnImportar.Enabled:= false;
  ...
  if (achei = false) then
    begin
      ...
      BEGIN
        ...
        UDeclaracao.Insere_Historico(usuario, 'REALIZOU PEDIDO DE COMPRA ' + EdtN_Pedido.Text + '.', TimeToStr(time), insercao, date);
      END
      else
        exit;
    end
    ...
    begin
      if (Confira_Pedido = true) then
      BEGIN
        Compra.Alterar(Compra);
        UDeclaracao.Insere_Historico(usuario, 'ALTEROU PEDIDO DE COMPRA ' + EdtN_Pedido.Text + '.', TimeToStr(time), alteracao, date);
      END
      else
        exit;
    end;
...
begin
  ...
  begin
    if (Confirma_Delete = true) then
      begin
        Itens_Compra.Excluir_Selecionado(Itens_Compra);
...
procedure TFrmCompra.EdtCodigo_FuncionarioKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if (key = vk_F3) then
  begin
...
procedure TFrmCompra.EdtFuncionarioKeyPress(Sender: TObject; var Key: Char);
begin
  Util.Somente_Apaga(key);
end;
...
begin
  ...
  begin
    ...
    begin
      Itens_Compra.Inserir(Itens_Compra);
      Atualiza_Dados;
      Calcula_Total;
      Limpa_Menor;
