...
type
  TFrmRel_Impressao_DAV = class(TForm)
    QuickRep1: TQuickRep;
...
var
  FrmRel_Impressao_DAV: TFrmRel_Impressao_DAV;
...
procedure TFrmRel_Impressao_DAV.FormCreate(Sender: TObject);
var
  i: integer;
begin
...
procedure TFrmRel_Impressao_DAV.QRBand1AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  LblQtde_Total.Caption:= FloatToStr(qtde_total);
end;

procedure TFrmRel_Impressao_DAV.QRDBText5Print(sender: TObject;
  var Value: string);
begin
  qtde_total:= qtde_total + StrToFloat(Value);
end;

procedure TFrmRel_Impressao_DAV.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
  var
    i: integer;
    texto: string;
