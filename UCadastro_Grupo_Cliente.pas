...
uses UDM, UChama_Grupo_Cliente, UDeclaracao;
...
begin
  ...
  BEGIN
    Grupo_Cliente.Excluir(Grupo_Cliente);
    //Cria_Arquivo_Atualiza_Dados('DGCli');
    UDeclaracao.Insere_Historico(usuario, 'EXCLUIU GRUPO DE CLIENTE ' + EdtDescricao.Text + '.', TimeToStr(time), exclusao, date);
    BBtnSalvar.Enabled:= false;
    BBtnExcluir.Enabled:= false;
    BBtnPesquisar.Enabled:= true;
    BBtnCancelar.Enabled:= false;
...
procedure TFrmCadastro_Grupo_Cliente.BBtnNovoClick(Sender: TObject);
begin
...
begin
  if (achei = false) then
    begin
      ...
      BEGIN
        Grupo_Cliente.Inserir(Grupo_Cliente);
        //Cria_Arquivo_Atualiza_Dados('IGCli');
        UDeclaracao.Insere_Historico(usuario, 'CADASTROU GRUPO DE CLIENTE ' + EdtDescricao.Text + '.', TimeToStr(time), insercao, date);
      END
      else
        exit;
    end
    ...
    begin
      if (Confira = true) then
      BEGIN
        Grupo_Cliente.Alterar(Grupo_Cliente);
        //Cria_Arquivo_Atualiza_Dados('AGCli');
        UDeclaracao.Insere_Historico(usuario, 'ALTEROU GRUPO DE CLIENTE ' + EdtDescricao.Text + '.', TimeToStr(time), alteracao, date);
      END
      else
        exit;
    end;
...
begin
  ...
  Util.Limpa_Campos(FrmCadastro_Grupo_Cliente);
  Util.Desabilita_Campos(FrmCadastro_Grupo_Cliente);
  BBtnNovo.Hint := hintNovo;
  BBtnSalvar.Hint := hintSalvar;
  BBtnExcluir.Hint := hintExcluir;
  BBtnCancelar.Hint := hintCancelar;
  BBtnFechar.Hint := hintFechar;
  BBtnPesquisar.Hint := hintPesquisar;
...
procedure TFrmCadastro_Grupo_Cliente.MEdtData_CadastroExit(Sender: TObject);
begin
