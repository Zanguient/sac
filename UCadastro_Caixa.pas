...
uses UDM, UChama_Caixa, UDeclaracao;
...
begin
  ...
  begin
    Caixa.Excluir(Caixa);
    UDeclaracao.Insere_Historico(usuario, 'EXCLUIU CAIXA ' + EdtCaixa.Text + '.', TimeToStr(time), exclusao, date);
    BBtnSalvar.Enabled:= false;
    BBtnExcluir.Enabled:= false;
    BBtnPesquisar.Enabled:= true;
    BBtnCancelar.Enabled:= false;
...
begin
  if (achei = false) then
    begin
      ...
      begin
        Caixa.Inserir(Caixa);
        UDeclaracao.Insere_Historico(usuario, 'CADASTROU CAIXA ' + EdtCaixa.Text + '.', TimeToStr(time), insercao, date);
      end
      else
        exit;
    end
    ...
    begin
      ...
      begin
        Caixa.Alterar(Caixa);
        UDeclaracao.Insere_Historico(usuario, 'ALTEROU CAIXA ' + EdtCaixa.Text + '.', TimeToStr(time), alteracao, date);
      end
      else
        exit;
    end;
