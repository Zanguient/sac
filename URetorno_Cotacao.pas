...
type
  TFrmRetorno_Cotacao = class(TForm)
    ...
    qryitens_retorno_cotacaoCodigo_Venda: TStringField;
...
uses UDM, UChama_Produto, UChama_Cliente, UChama_Condicao_Pagamento,
  UDeclaracao, UChama_Fornecedor, UChama_Funcionario;
...
begin
  ...
  begin
    if (qryitens_cotacao.IsEmpty = false) then
      Itens_Retorno_Cotacao.Excluir(Itens_Retorno_Cotacao);
  end;
...
begin
  ...
  begin
    Retorno_Cotacao.Excluir(Retorno_Cotacao);
    Itens_Retorno_Cotacao.Excluir(Itens_Retorno_Cotacao);
    UDeclaracao.Insere_Historico(usuario, 'EXCLUIU O RETORNO DA COTAÇÃO ' + EdtN_Cotacao.Text + '.', TimeToStr(time), exclusao, date);
    BBtnSalvar.Enabled:= false;
    BBtnExcluir.Enabled:= false;
    BBtnCancelar.Enabled:= false;
    Util.Desabilita_Campos(FrmRetorno_Cotacao);
...
begin
  if (achei = false) then
    begin
      ...
      BEGIN
        Retorno_Cotacao.Inserir(Retorno_Cotacao);
        UDeclaracao.Insere_Historico(usuario, 'CADASTROU O RETORNO DA COTAÇÃO ' + EdtN_Cotacao.Text + '.', TimeToStr(time), insercao, date);
      END
      else
        exit;
    end
    ...
    begin
      if (Confira_Pedido = true) then
      begin
        Retorno_Cotacao.Alterar(Retorno_Cotacao);
        UDeclaracao.Insere_Historico(usuario, 'ALTEROU O RETORNO DA COTAÇÃO ' + EdtN_Cotacao.Text + '.', TimeToStr(time), alteracao, date);
      end
      else
        exit;
    end;
...
begin
  ...
  begin
    if (Confirma_Delete = true) then
      begin
        Itens_Retorno_Cotacao.Excluir_Selecionado(Itens_Retorno_Cotacao);
...
procedure TFrmRetorno_Cotacao.EdtCodigo_FuncionarioKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if (key = vk_F3) then
  begin
...
procedure TFrmRetorno_Cotacao.EdtFuncionarioKeyPress(Sender: TObject; var Key: Char);
begin
  Util.Somente_Apaga(key);
end;
...
begin
  ...
  if (qAux2.FieldByName('CPF').AsString <> '') then
    FrmRetorno_Cotacao.LblCPF.Caption:= qAux2.FieldByName('CPF').AsString
  else if (qAux2.FieldByName('CNPJ').AsString <> '') then
    FrmRetorno_Cotacao.LblCPF.Caption:= qAux2.FieldByName('CNPJ').AsString
  else
    FrmRetorno_Cotacao.LblCPF.Caption:= '';

  FrmRetorno_Cotacao.LblCidade.Caption:= qAux2.FieldByName('Descricao').AsString;
  FrmRetorno_Cotacao.LblSetor.Caption:= qAux2.FieldByName('Descricao_1').AsString;
  FrmRetorno_Cotacao.LblEndereco.Caption:= qAux2.FieldByName('Descricao_2').AsString;
  FrmRetorno_Cotacao.LblTipo_Logradouro.Caption:= qAux2.FieldByName('Tipo_Logradouro').AsString;
  FrmRetorno_Cotacao.LblUF.Caption:= qAux2.FieldByName('UF').AsString;
  FrmRetorno_Cotacao.LblTelefone.Caption:= qAux2.FieldByName('Telefone').AsString;
  FrmRetorno_Cotacao.LblNumero.Caption:= qAux2.FieldByName('Numero').AsString;
