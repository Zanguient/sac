unit URel_Contagem_Estoque_Marca;
...
type
  TFrmRel_Contagem_Estoque_Marca = class(TForm)
    QuickRep1: TQuickRep;
...
var
  FrmRel_Contagem_Estoque_Marca: TFrmRel_Contagem_Estoque_Marca;
...
procedure TFrmRel_Contagem_Estoque_Marca.FormCreate(Sender: TObject);
begin
//  if (FrmEscolha_Impressao.preview = true) then
    QuickRep1.Preview;
end;

procedure TFrmRel_Contagem_Estoque_Marca.QuickRep1BeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  ...
  begin
    close;
    sql.clear;
    sql.add('select P.Codigo_Venda, P.Descricao, P.Estoque, M.Descricao as Marca, P.Codigo_Marca, F.Nome_Fantasia from Produto P');
    sql.add('left join Marca M on (P.Codigo_Marca = M.Codigo)');
    sql.add('left join Fornecedor F on (P.Codigo_Fornecedor = F.Codigo)');

    sql.add('where P.Codigo_Empresa = :Empresa and P.Status = :Status and P.Tipo = :Tipo order by M.Descricao, P.Descricao');
