...
type
  TFrmConsulta_Retorno_Cotacao_Preco = class(TForm)
    ...
    qryconsulta_cotacaoCodigo: TAutoIncField;
    ...
    qryconsulta_cotacaoCodigo_Funcionario: TIntegerField;
...
uses UDM, UChama_Funcionario, UChama_Fornecedor,
  URel_Consulta_Retorno_Cotacao_Preco;
...
begin
  if (key = vk_f3) then
    begin
      if (RGConsulta_Por.ItemIndex = 3) or (RGConsulta_Por.ItemIndex = 5) or (RGConsulta_Por.ItemIndex = 6) then
      begin
        Application.CreateForm(TFrmChama_Funcionario, FrmChama_Funcionario);
        FrmChama_Funcionario.ShowModal;
        FrmChama_Funcionario.Free;
      end
      else if (RGConsulta_Por.ItemIndex = 4) or (RGConsulta_Por.ItemIndex = 7) or (RGConsulta_Por.ItemIndex = 8) then
      begin
...
procedure TFrmConsulta_Retorno_Cotacao_Preco.RGConsulta_PorClick(
  Sender: TObject);
begin
  if (RGConsulta_Por.ItemIndex = 3) or (RGConsulta_Por.ItemIndex = 5) or (RGConsulta_Por.ItemIndex = 6) then
  begin
    Label2.Caption:= 'Funcionário';
  end
  else if (RGConsulta_Por.ItemIndex = 4) or (RGConsulta_Por.ItemIndex = 7) or (RGConsulta_Por.ItemIndex = 8) then
  begin
